...
The easiest way to try MuseTalk is the one-click Colab notebook:

1. Click [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/MubeenAmjad205/MuseTalk/blob/main/notebooks/MuseTalk_Colab.ipynb)
2. In Colab, go to **Runtime > Change runtime type** and select a **T4 GPU** (or better).
3. *(Optional)* Add a Colab secret named `HF_TOKEN` with your Hugging Face token, and tick `save_models_to_drive` to cache the weights in Google Drive.
4. Run the single cell. It installs everything, downloads the weights, and prints a public `xxxx.gradio.live` link.
